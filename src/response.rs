...
impl TryFrom<Response> for generated::Reply {
    ...
    fn try_from(response: Response) -> Result<Self> {
        ...
        Ok(match response {
            ...
            Response::Theme { mode } => Self::Theme(theme(mode)),
            Response::Warmth { kelvin } => Self::Warmth(i64::from(kelvin.as_u16())),
            Response::Brightness { percent } => Self::Brightness(i64::from(percent.as_u8())),
            Response::State { theme: value, kelvin, percent } => Self::State(generated::ReplyState { theme_mode: theme(value), first_integer: i64::from(kelvin.as_u16()), second_integer: i64::from(percent.as_u8()) }),
            Response::SolarClock { utc_offset_seconds, equation_of_time_valid_until_unix_seconds } => {
                Self::SolarClock(generated::ReplySolarClock { first_integer: i64::from(utc_offset_seconds), second_integer: equation_of_time_valid_until_unix_seconds })
            }
